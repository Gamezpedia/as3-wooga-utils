package com.flashdynamix.utils {
	...
	public class SWFProfiler {
		...
		private static var showCI:ContextMenuItem;
		private static var resetCI:ContextMenuItem;

		public static function init(swf:Stage, context:InteractiveObject, autoShow:Boolean = false):void {
			if (inited) {
				return;
			}
			...
			showCI = new ContextMenuItem("Show Profiler", true);
			addEvent(showCI, ContextMenuEvent.MENU_ITEM_SELECT, onShow);

			resetCI = new ContextMenuItem("Reset Profiler", true);
			addEvent(resetCI, ContextMenuEvent.MENU_ITEM_SELECT, onReset);

			cm.customItems = [showCI, resetCI];
		...
		private static function onReset(e:ContextMenuEvent):void {
			reset();
		}


		private static function onShow(e:ContextMenuEvent):void {
			if (!displayed) {
				show();
			} else {
				hide();
